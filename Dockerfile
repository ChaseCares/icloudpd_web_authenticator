...
COPY icloudpd_web_authenticator icloudpd_web_authenticator
COPY assets assets
...
COPY --from=builder --chown=sid:sid /app/icloudpd_web_authenticator/static /app/icloudpd_web_authenticator/static
COPY ["Pipfile", "shell_scripts/auto_pipenv.sh", "./"]
RUN pip install --no-cache pipenv
RUN pipenv install
